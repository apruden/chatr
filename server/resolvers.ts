...
const saveAccount: (AccountInput) => Promise<Account> = async (newAccount: AccountInput) => {
...
export class RegistrationResolver {
  ...
  async register(@Ctx() context: any, @Arg("data") data: RegistrationInput): Promise<Registration> {
    const account = await saveAccount(data.account)
...
export class AccountResolver {
  ...
  async updateAccount(
    @Arg('data') newAccount: AccountInput
  ): Promise<Account> {
...
export class RelationResolver {
  ...
  async blockUser(
    ...
    const {id: me} = context.user
    const type = 'blocked'
    await pool.query(`INSERT INTO relations(subject, target, type) VALUE ($1, $2, $3)`, [me, id, type])
